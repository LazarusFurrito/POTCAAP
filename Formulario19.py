...
class ExcelExporter:
    ...
    def exportar_a_excel(self, archivo_salida="registros_medicos.xlsx"):
        ...
        try:
            ...
            column_order = [
                'id_registro', 'nombres', 'apellidos', 'fecha_registro',
                'altura', 'peso', 'temperatura', 'cintura', 'cadera',
                'presion_arterial', 'bpm', 'spo2'
            ]
            
            # Filtrar solo las columnas que existen
            columnas_existentes = [col for col in column_order if col in df.columns]
            ...
            with pd.ExcelWriter(archivo_salida, engine='openpyxl') as writer:
                ...
                # Obtener el libro y hoja de trabajo
                workbook = writer.book
                worksheet = writer.sheets['Registros Médicos']
    ...
    def exportar_a_excel_con_datos(self, registros, archivo_salida):
        ...
        try:
            ...
            # Mismo procesamiento que en exportar_a_excel
            column_order = [
                'id_registro', 'nombres', 'apellidos', 'fecha_registro',
                'altura', 'peso', 'temperatura', 'cintura', 'cadera',
                'presion_arterial', 'bpm', 'spo2'
...
class ExportarExcelDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.exporter = ExcelExporter(DatabaseManager())
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        titulo = QLabel("Exportar Registros a Excel")
        titulo.setStyleSheet("font-size: 16px; font-weight: bold; color: #2c3e50; margin: 10px;")
        titulo.setAlignment(Qt.AlignCenter)
        layout.addWidget(titulo)
        
        instruccion = QLabel("Seleccione el tipo de exportación:")
        instruccion.setStyleSheet("font-size: 12px; margin: 10px;")
        layout.addWidget(instruccion)
        
        # Botones de opción
        self.boton_todos = QPushButton("📊 Exportar Todos los Registros")
        self.boton_todos.setStyleSheet("QPushButton { background-color: #27ae60; color: white; font-weight: bold; padding: 10px; }")
        self.boton_todos.clicked.connect(self.exportar_todos)
        layout.addWidget(self.boton_todos)
        
        self.boton_filtrados = QPushButton("📅 Exportar por Fecha")
        self.boton_filtrados.setStyleSheet("QPushButton { background-color: #3498db; color: white; font-weight: bold; padding: 10px; }")
        self.boton_filtrados.clicked.connect(self.exportar_por_fecha)
        layout.addWidget(self.boton_filtrados)
        
        # Área de información
        self.info_label = QLabel("")
        self.info_label.setStyleSheet("font-size: 11px; color: #7f8c8d; margin: 10px;")
        self.info_label.setWordWrap(True)
        layout.addWidget(self.info_label)
        ...
        self.boton_abrir_carpeta.setStyleSheet("QPushButton { background-color: #95a5a6; color: white; font-weight: bold; }")
        self.boton_abrir_carpeta.clicked.connect(self.abrir_carpeta)
        ...
        botones_layout.addWidget(self.boton_abrir_carpeta)
        botones_layout.addWidget(self.boton_cerrar)
        
        layout.addLayout(botones_layout)
        self.setLayout(layout)
        
        self.archivo_exportado = None
        
    def exportar_todos(self):
        archivo = "registros_medicos_completos.xlsx"
        exito, mensaje = self.exporter.exportar_a_excel(archivo)
        
        if exito:
            self.info_label.setText(f"✅ {mensaje}\n\nUbicación: {os.path.abspath(archivo)}")
            self.info_label.setStyleSheet("font-size: 11px; color: #27ae60; margin: 10px;")
            self.archivo_exportado = archivo
            self.boton_abrir_carpeta.setEnabled(True)
        else:
            self.info_label.setText(f"❌ {mensaje}")
            self.info_label.setStyleSheet("font-size: 11px; color: #e74c3c; margin: 10px;")
    
    def exportar_por_fecha(self):
        # Diálogo simple para fechas (puedes mejorarlo)
        fecha_inicio, ok1 = QInputDialog.getText(self, "Fecha Inicio", "Ingrese fecha inicio (YYYY-MM-DD):")
        if not ok1:
            return
            
        fecha_fin, ok2 = QInputDialog.getText(self, "Fecha Fin", "Ingrese fecha fin (YYYY-MM-DD):")
        if not ok2:
            return
        
        archivo = f"registros_{fecha_inicio}_a_{fecha_fin}.xlsx"
        exito, mensaje = self.exporter.exportar_por_fecha(fecha_inicio, fecha_fin, archivo)
    ...
    def abrir_carpeta(self):
        if self.archivo_exportado and os.path.exists(self.archivo_exportado):
            os.startfile(os.path.dirname(os.path.abspath(self.archivo_exportado)))
...
class MenuRegistros(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.setFixedSize(400, 500)  # Aumenta la altura para el nuevo botón
        ...
        instrucciones = QLabel("Presione 1-7 para navegar o use el mouse\nF1-F7 también funcionan")
        ...
        self.botones_menu = [
            ...
            ("5. Exportar a Excel", self.exportar_excel),  # NUEVO BOTÓN
            ("6. Información del Proyecto", self.mostrar_informacion),
            ("7. Salir", self.salir)
        ]
        
        for texto, funcion in self.botones_menu:
            boton = QPushButton(texto)
            boton.setFixedSize(250, 45)
            if texto == "7. Salir":
                boton.setStyleSheet("QPushButton { background-color: #ff6b6b; color: white; font-weight: bold; }")
            elif texto == "6. Información del Proyecto":
                boton.setStyleSheet("QPushButton { background-color: #17a2b8; color: white; font-weight: bold; }")
            elif texto == "5. Exportar a Excel":  # Estilo para el nuevo botón
                boton.setStyleSheet("QPushButton { background-color: #28a745; color: white; font-weight: bold; }")
            else:
    ...
    def exportar_excel(self):
        exportar_dialog = ExportarExcelDialog(self)
        exportar_dialog.exec_()
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        elif key == Qt.Key_5 or key == Qt.Key_F5:  # NUEVA TECLA
            self.exportar_excel()
        elif key == Qt.Key_6 or key == Qt.Key_F6:
            self.mostrar_informacion()
        elif key == Qt.Key_7 or key == Qt.Key_F7:  # Actualizada
            self.salir()
        else:
            super().keyPressEvent(event)
